    ...
    <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-6 md:mb-8">
            <div class="flex flex-col md:flex-row md:items-center gap-3 md:gap-4 mb-4">
                ...
                <div class="md:ml-2">
                    <h1 class="text-xl md:text-2xl font-bold text-gray-900">Edit Mata Pelajaran</h1>
                    <p class="text-gray-600 mt-1 text-sm md:text-base">Perbarui informasi mata pelajaran {{ $mataPelajaran->nama_mapel }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <!-- Form Header -->
            <div class="px-4 md:px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-blue-50 to-indigo-100">
                <h2 class="text-lg font-semibold text-gray-900">Formulir Edit Mata Pelajaran</h2>
                <p class="text-sm text-gray-600 mt-1">Perbarui data mata pelajaran di bawah ini</p>
            </div>

            <div class="p-4 md:p-6">
                <form id="edit-mapel-form" action="{{ route('mata-pelajaran.update', $mataPelajaran) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-5 md:mb-6">
                        ...
                        <p class="mt-1 text-xs text-gray-500">Nama lengkap mata pelajaran (maksimal 100 karakter)</p>
                        @error('nama_mapel')
                    ...
                    <div class="mb-5 md:mb-6">
                        ...
                        <div class="relative">
                            <input type="text" 
                                   ...
                                   required
                                   class="w-full px-4 py-3 pl-10 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors uppercase"
                    ...
                    <div class="mb-6 md:mb-8">
                        ...
                        <div class="relative">
                            <textarea name="deskripsi" 
                                      id="deskripsi"
                                      rows="4"
                                      class="w-full px-4 py-3 pl-10 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors resize-none"
                                      placeholder="Deskripsi singkat tentang mata pelajaran, tujuan pembelajaran, atau informasi penting lainnya">{{ old('deskripsi', $mataPelajaran->deskripsi) }}</textarea>
                        ...
                        <div class="flex justify-between items-center mt-1">
                            ...
                            <span id="char-count" class="text-xs text-gray-400">{{ strlen(old('deskripsi', $mataPelajaran->deskripsi)) }}/500</span>
                        </div>
                        @error('deskripsi')
                            <div class="mt-2 flex items-center text-sm text-red-600">
                                <i class="fas fa-exclamation-circle mr-1.5"></i>
                    ...
                    <!-- Preview Card -->
                    <div class="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg transition-all duration-300">
                        ...
                        <div class="bg-white rounded-lg p-4 border border-blue-100">
                            <div class="flex justify-between items-start mb-2">
                                <div>
                                    <h4 id="preview-nama" class="font-semibold text-gray-900 text-lg">{{ $mataPelajaran->nama_mapel }}</h4>
                                    <p class="text-sm text-gray-500">Kode: <span id="preview-kode" class="font-mono">{{ $mataPelajaran->kode_mapel }}</span></p>
                                </div>
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                                    <i class="fas fa-layer-group mr-1 text-xs"></i>
                                    {{ $mataPelajaran->komponen_nilai_count }} Komponen
                                </span>
                            </div>
                            <p id="preview-deskripsi" class="text-sm text-gray-600 mt-2">
                    ...
                    <div class="flex flex-col sm:flex-row gap-3 pt-4 border-t border-gray-200">
                        <button type="submit" 
                                id="submit-button"
                                class="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg transition-colors flex items-center justify-center focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transform hover:-translate-y-0.5 transition-transform">
        ...
        <div class="mt-6 bg-gray-50 border border-gray-200 rounded-xl p-4">
            <div class="flex items-start">
                ...
                <div>
                    <p class="text-sm font-medium text-gray-800 mb-2">Informasi Saat Ini</p>
                    <div class="text-xs text-gray-600 space-y-1">
                        <p>Nama: <span class="font-semibold">{{ $mataPelajaran->nama_mapel }}</span></p>
                        <p>Kode: <span class="font-semibold font-mono">{{ $mataPelajaran->kode_mapel }}</span></p>
                        <p>Komponen Penilaian: <span class="font-semibold">{{ $mataPelajaran->komponen_nilai_count }}</span></p>
                        <p>Dibuat: <span class="font-semibold">{{ $mataPelajaran->created_at->format('d M Y') }}</span></p>
                        @if($mataPelajaran->updated_at->gt($mataPelajaran->created_at))
                        <p>Terakhir Diupdate: <span class="font-semibold">{{ $mataPelajaran->updated_at->format('d M Y') }}</span></p>
                        @endif
                    </div>
                </div>
            </div>
            ...
            <div class="flex items-start">
                ...
                <div>
                    ...
                    <ul class="text-xs text-yellow-700 space-y-1 list-disc list-inside">
                        <li>Perubahan pada kode mata pelajaran dapat mempengaruhi data yang sudah terkait</li>
                        <li>Pastikan kode mata pelajaran tetap unik setelah diubah</li>
                        <li>Komponen penilaian yang sudah dibuat akan tetap terkait dengan mata pelajaran ini</li>
                    </ul>
                </div>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    const form = document.getElementById('edit-mapel-form');
    ...
    function updatePreview() {
        // Update nama
        const namaValue = namaMapelInput.value.trim();
        previewNama.textContent = namaValue || '{{ $mataPelajaran->nama_mapel }}';
        
        // Update kode
        const kodeValue = kodeMapelInput.value.trim();
        previewKode.textContent = kodeValue || '{{ $mataPelajaran->kode_mapel }}';
        
        // Update deskripsi
        const deskripsiValue = deskripsiTextarea.value.trim();
        if (deskripsiValue) {
            previewDeskripsi.textContent = deskripsiValue;
    ...
    function hasFormChanges() {
        const originalNama = '{{ $mataPelajaran->nama_mapel }}';
        const originalKode = '{{ $mataPelajaran->kode_mapel }}';
        const originalDeskripsi = '{{ $mataPelajaran->deskripsi }}';
        
        return namaMapelInput.value !== originalNama || 
               kodeMapelInput.value !== originalKode || 
    ...
    form.addEventListener('submit', function(e) {
        ...
        if (!hasFormChanges()) {
            ...
            if (confirm('Tidak ada perubahan yang dilakukan. Tetap lanjutkan?')) {
                // Allow form submission even without changes
                return true;
            }
            return false;
        }
        ...
        submitButton.innerHTML = '<i class="fas fa-spinner fa-spin mr-2"></i><span>Memperbarui...</span>';
        submitButton.classList.remove('bg-blue-600', 'hover:bg-blue-700', 'hover:-translate-y-0.5');
